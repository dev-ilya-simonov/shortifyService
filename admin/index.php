...
<body>
    <?if($USER->isAuth()):?>
        ...
        ?>
        <h1>Админка</h1>
        <a href="/admin/links_visits/">Переходы</a>
        ...
        <table cellpadding="10" cellspacing="5">
            ...
            ?>
                    <tr>
                        <td><?=$arItem['id']?></td>
                        <td><input class="full-link" type="text" data-type="change" readonly data-ajax data-id="<?=$arItem['id']?>" name="initial_link" value="<?=$arItem['initial_link']?>"></td>
                        <td><a href="<?=$arItem['full_short_link']?>"><?=$arItem['full_short_link']?></a></td>
                        <td><?=$arItem['date']?></td>
                        <td><?=$arItem['ip']?></td>
                        <td><p data-ajax data-type="delete" data-id="<?=$arItem['id']?>">x</p></td>
                    </tr>
            <?
                endforeach;
            ?>
